...
const Package2 = require("../modles/Package2");
...
router.get('/',async(req,res) => {
    try{
           const package2 = await Package2.find()
           res.json(package2)
    }catch(err){
        res.send('Error ' + err)
    }
...
router.get('/:id', async(req,res) => {
    try{
           const package2 = await Package2.findById(req.params.id)
           res.json(package2)
    }catch(err){
        res.send('Error ' + err)
    }
...
router.patch('/:id',async(req,res)=> {
    try{
        const package2 = await Package2.findById(req.params.id) 
        package2.name = req.body.name
        const a1 = await package2.save()
        res.json(a1)   
    }catch(err){
        res.send('Error')
    }
    ...
    router.delete('/:id',async(req,res)=> {
        try{
           // const addresscheck = await Addresscheck.findById(req.params.id) 
            const package2 =await Package2.findByIdAndDelete(req.params.id);
